...
金融是强合规领域：给不出推理过程的答案，业务上不可用。本项目在 **6×RTX4090** 上把 Qwen3-8B 后训练成会"先推理、后作答"的金融模型。

SFT 阶段四个基准全部提升，均值 **+9.66 pp**，通用数学与科学能力未退化。随后的 GRPO 阶段净增益约等于零：`--ref_adapters` 把参考模型锚死在 SFT 检查点上，KL 全程 ≈ 0，策略几乎没有离开起点——策略不动，奖励设计再好也无从生效。

全过程的失败、归因与修复记录在 [EXPERIMENT_LOG.md](EXPERIMENT_LOG.md)（含三处事后复核的更正），四个基准的原始评测结果在 [eval/](eval/)。
