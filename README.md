...
金融是强合规领域：给不出推理过程的答案，业务上不可用。本项目在 **6×RTX4090** 上把 Qwen3-8B 后训练成会"先推理、后作答"的金融模型——SFT 阶段四个基准全部提升，均值 **+9.66 pp**；紧接着的 GRPO 阶段，净增益约等于零。

值得读的是后半句。RL 没能带来增益的原因被定位到一行参数：`--ref_adapters` 把参考模型锚死在 SFT 检查点上，KL 全程 ≈ 0，策略几乎没有离开起点——策略不动，奖励设计再好也无从生效。这个负结果连同全过程的"现象 → 归因 → 修复"日志一并公开，其中包括三处事后复核发现自己记错、就地更正而非抹去的地方。

仓库内每一个结论都能回溯到对应的评测 artifact。
