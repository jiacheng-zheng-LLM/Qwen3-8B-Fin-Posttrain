...
**用 LoRA 在 Qwen3-8B 上做两阶段后训练(SFT→GRPO):SFT 4 基准全涨、均值 +9.6pp(能力增量的主体);GRPO 在客观难例筛选 + 双奖励 + 分离部署下跑通 1210 步,但对已强的多任务 SFT 净增益≈0(均值 −0.1pp)——这是诚实结果,根因是 ref_adapters+β0.04 强正则使 KL≈0(策略几乎未移动),换来的是跨任务零遗忘(FinQA +1.3、MATH ±0.0)。** 绝对分受 LoRA(非全参)与判分口径影响,不宜横向对标其他实现。
...
- **场景特殊性**:我们 SFT 是**多任务**(4基准全涨),而 GRPO 奖励**只有 CFLUE** → 另3任务有遗忘风险;
...
|---|---|---|---|---|---|
| CFLUE(n=800) | 48.0 | 53.9 | **54.8** | +5.9 | **+0.9** |
...
2. **分域自洽**:奖励域 CFLUE 微涨 +0.9(策略被锁,涨不动);同域 FinQA +1.3、无关域 MATH ±0.0 → **零遗忘**,印证 ref_adapters 护多任务的设计意图;
...
3. **难例数据**:客观 c/k 难例是本项目自建,判据与来源均自定;GRPO 用已 SFT 训过的 cflue_mcq 题(标准做法);
4. **口径**:LoRA(非全参)、基座 Qwen3-8B、max_length 5120(SFT)、CFLUE 抽样800、GRPO num_generations 4(显存所限)。
...
## 六、可复现资产
- 数据管线/评测/难例筛选/reward:`scripts/{eval_fin,build_hardcase_rl}.py`、`plugin/fin_orm.py`;
