...
DATA="$ROOT/data/grpo_hardcase_smoke.jsonl"; PORT=8137
SW="${SWIFT_BIN:-swift}"
RUNDIR="${RUN_DIR:-$ROOT/runs}"; mkdir -p "$RUNDIR"
SVLOG="$RUNDIR/rollout_server.log"; TRLOG="$RUNDIR/grpo_smoke_trainer.log"; OUT="$RUNDIR/grpo-smoke"
...
env CUDA_VISIBLE_DEVICES=3,4 NPROC_PER_NODE=2 $COMMON $SW rlhf --rlhf_type grpo   --model "$BASE" --adapters "$CKPT"   --lora_rank 32 --lora_alpha 64 --target_modules all-linear   --external_plugins "$ROOT/plugin/fin_orm.py"   --reward_funcs fin_acc fin_format --reward_weights 1.0 0.1   --dataset "$DATA" --torch_dtype bfloat16 --attn_impl sdpa   --use_vllm true --vllm_mode server --vllm_server_host 127.0.0.1 --vllm_server_port $PORT   --vllm_server_timeout 600 --vllm_server_pass_dataset true   --num_generations 4 --max_completion_length 1024   --per_device_train_batch_size 1 --gradient_accumulation_steps 4   --learning_rate 1e-6 --beta 0.04 --max_steps 8 --max_length 4096   --gradient_checkpointing true --logging_steps 1 --save_strategy no   --output_dir "$OUT" --report_to tensorboard >> "$TRLOG" 2>&1
