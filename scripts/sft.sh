...
# 4 卡 DDP（按你的机器改 CUDA_VISIBLE_DEVICES 与 NPROC_PER_NODE）
GPUS="${SFT_GPUS:-0,2,3,4}"
NPROC="${SFT_NPROC:-4}"
...
env CUDA_VISIBLE_DEVICES="$GPUS" NPROC_PER_NODE="$NPROC" \
  ...
  "$SW" sft \
    ...
    --train_type lora --lora_rank 32 --lora_alpha 64 --lora_dropout 0.05 \
